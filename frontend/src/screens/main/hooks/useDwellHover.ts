...
export const useDwellHover = () => {
  ...
  const updateDwellEnabled = useCallback(
    async (enabled: boolean) => {
      const next = { ...params, clicking: { ...params.clicking, dwellEnabled: enabled } };
      setParams(next);
      try {
  ...
  const onHoverStart = useCallback(() => {
    if (params.clicking.dwellEnabled || timerRef.current) return;
    timerRef.current = window.setTimeout(() => {
      timerRef.current = null;
      void updateDwellEnabled(true);
    }, 500);
  }, [params.clicking.dwellEnabled, updateDwellEnabled]);
